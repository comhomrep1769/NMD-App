  ...
  const loginClient = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user.role !== "client") {
        setError("This login area is for client accounts. Admins use /admin and employees use /employee.");
        return;
      }
      ...
      <section className="loginCard" style={{ maxWidth: 1180 }}>
        <div className="panelHeader">
          ...
          <div className="buttonRow">
            <button className="secondaryButton" onClick={onAdminLogin}>
              Admin
            </button>

            <button className="secondaryButton" onClick={onEmployeeLogin}>
              Employee
            </button>
          </div>
        </div>
