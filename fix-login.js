...
function LoginForm({ portalRole }: { portalRole: string }) {
  ...
  const subtitle =
    portalRole === 'client' ? 'Access your quotes, invoices, appointments, and service requests.' :
    portalRole === 'employee' ? 'Access assigned jobs, schedule, treatments, and chat.' :
    'Access operations, clients, quotes, invoices, scheduling, and Guru.'
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div>
            ...
            <input
              ...
              style={{ width: '100%', padding: '0.6rem 0.85rem', borderRadius: 8, border: '1.5px solid #dde4ef', fontSize: '0.875rem', outline: 'none', fontFamily: 'DM Sans, sans-serif', color: '#0e1117', background: '#f4f7fb', boxSizing: 'border-box' as const }}
            />
          </div>
          <div>
