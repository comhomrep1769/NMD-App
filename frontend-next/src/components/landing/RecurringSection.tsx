  ...
  return (
    <section className="bg-white px-4 py-24 sm:px-[65px]" id="recurring">
      <div className="mx-auto max-w-[1440px]">
        ...
        <div className="grid-recurring grid grid-cols-6 gap-4">
          {plans.map((plan) => (
            <div
              key={plan.freq}
              className={
                plan.featured
                  ? 'flex flex-col gap-2.5 rounded-xl bg-teal-700 px-4 py-5'
                  : 'flex flex-col gap-2.5 rounded-xl border border-gray-200 bg-[#F8FAF9] px-4 py-5'
              }
              style={plan.featured ? { position: 'relative' } : undefined}
            >
              {plan.featured && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-[#0C5A54] px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-white">
                  Popular
                </div>
              )}
              ...
              <div
                className={
                  plan.featured
                    ? 'inline-flex w-fit items-center rounded-md bg-white/[0.18] px-2 py-[3px] text-[11px] font-bold text-white'
                    : 'inline-flex w-fit items-center rounded-md bg-[#F0FDF9] px-2 py-[3px] text-[11px] font-bold text-[#0F766E]'
                }
              >
                20% off
              </div>
        ...
        <div className="mt-8 flex items-center gap-5 rounded-xl border border-blue-100 bg-blue-50 p-6">
