            ...
            return (
              <div key={item.title} className="overflow-hidden rounded-xl border border-gray-200 bg-white">
                <div className="relative h-[280px] overflow-hidden">
                  <img src={imgUrl} alt={`${showAfter ? 'After' : 'Before'} — ${item.title}`} className="h-full w-full object-cover" style={showAfter ? { filter: 'brightness(1.08) saturate(1.12)' } : { filter: 'brightness(0.7) saturate(0.4) sepia(0.25)' }} />
                  <div className="pointer-events-none absolute inset-0" style={{ background: showAfter ? 'linear-gradient(to top, rgba(0,0,0,0.18) 0%, transparent 55%)' : 'linear-gradient(to top, rgba(0,0,0,0.45) 0%, transparent 55%)' }} />
                  <div className="absolute bottom-3.5 right-3.5 flex gap-0.5 rounded-full bg-black/65 p-[3px]">
                    <button onClick={() => toggle(i, false)} className={!showAfter ? 'rounded-full bg-white/90 px-3 py-1.5 text-xs font-semibold text-gray-900' : 'rounded-full px-3 py-1.5 text-xs font-normal text-white/70'}>Before</button>
                    <button onClick={() => toggle(i, true)} className={showAfter ? 'rounded-full bg-white/90 px-3 py-1.5 text-xs font-semibold text-gray-900' : 'rounded-full px-3 py-1.5 text-xs font-normal text-white/70'}>After</button>
                  </div>
                </div>
