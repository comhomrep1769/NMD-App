...
const LOGIN_HERO_DEFAULT = 'https://images.unsplash.com/photo-1621905251918-48416bd8575a?auto=format&fit=crop&w=1800&q=80'

function getPortalPath(role: string, mustChangePassword?: boolean) {
  const r = role.toLowerCase()
  if (mustChangePassword) {
    if (r === 'employee') return '/employee/change-password'
    if (r === 'admin' || r === 'technical') return '/admin/change-password'
  }
  if (r === 'technical' || r === 'admin') return '/dashboard/admin'
...
function LoginForm({ portalRole }: { portalRole: string }) {
  ...
  const [heroImg, setHeroImg] = useState(LOGIN_HERO_DEFAULT)

  useEffect(() => {
    const API = process.env.NEXT_PUBLIC_API_URL || ''
    fetch(`${API}/api/site-content`)
      .then(r => r.json())
      .then(d => { if (d.content?.['login.hero_image_url']) setHeroImg(d.content['login.hero_image_url']) })
      .catch(() => {})
  }, [])
  ...
  const roleLabel =
    ...
    portalRole === 'technical' ? 'Technical Personnel' : 'Admin'
  ...
  const badge =
    ...
    portalRole === 'technical' ? { bg: '#FEF2F2', border: '#FECACA', color: '#DC2626', text: 'Restricted Access — Technical Personnel Only' } :
  ...
  return (
    <div className="flex h-[100vh] overflow-hidden">
      ...
      <div className="nmd-login-brand flex flex-col justify-end relative overflow-hidden" style={{ flex: '0 0 55%', background: '#0A1F1C', padding: 48 }}>
        <div className="absolute inset-0" style={{ backgroundImage: 'repeating-linear-gradient(135deg, rgba(255,255,255,0.015) 0, rgba(255,255,255,0.015) 1px, transparent 0, transparent 50%)', backgroundSize: '28px 28px' }} />
        <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url('${heroImg}')`, animation: 'nmdLoginHeroZoom 14s ease-out forwards' }} />
        ...
        <div className="relative z-[2]">
          ...
          <div className="text-[9px] font-bold tracking-[0.14em] uppercase mb-[20px]" style={{ color: '#34D399' }}>SERVICES LLC &mdash; {roleLabel.toUpperCase()} PORTAL</div>
          <p className="text-[14px] leading-[1.65] max-w-[340px] m-0" style={{ color: 'rgba(255,255,255,0.55)' }}>{heroDescription}</p>
        </div>
      </div>
      ...
      <div className="nmd-login-right flex-1 bg-white flex items-center justify-center px-[48px]">
        <div className="w-full max-w-[360px]">
          {badge && (
            <div className="inline-flex items-center gap-[6px] rounded-[7px] px-[12px] py-[8px] mb-[24px]" style={{ background: badge.bg, border: `1px solid ${badge.border}` }}>
          ...
          {error && (
            <div className="bg-[#FEF2F2] border border-[#FECACA] rounded-[8px] px-[14px] py-[10px] mb-[16px] text-[13px] text-[#B91C1C]">{error}</div>
          )}
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-[14px]">
            <div>
              ...
              <input type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" className="w-full px-[14px] py-[11px] border border-[#E5E7EB] rounded-[8px] text-[14px] outline-none" />
            </div>
            <div>
              <label className="block text-[13px] font-semibold text-[#374151] mb-[6px]">Password</label>
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} required value={password} onChange={e => setPassword(e.target.value)} placeholder="Enter your password" className="w-full px-[14px] py-[11px] pr-[40px] border border-[#E5E7EB] rounded-[8px] text-[14px] outline-none" />
                <button type="button" onClick={() => setShowPassword(p => !p)} className="absolute right-[10px] top-1/2 -translate-y-1/2 bg-transparent border-none cursor-pointer p-0 flex items-center" style={{ color: '#9CA3AF' }} aria-label={showPassword ? 'Hide password' : 'Show password'}>
                  {showPassword ? (
            ...
            <button type="submit" disabled={loading} className="w-full text-[15px] font-semibold py-[12px] rounded-[8px] border-none mt-[2px]" style={{ background: loading ? '#D1FAE5' : '#0F766E', color: loading ? '#0F766E' : '#fff', cursor: loading ? 'not-allowed' : 'pointer' }}>
              {loading ? 'Signing in...' : `Sign In to ${roleLabel} Portal`}
            </button>
          </form>

          {portalRole === 'client' && (
            <div style={{ marginTop: '14px', padding: '0.85rem 1rem', borderRadius: 8, background: '#F0FDF9', border: '1px solid #D1FAE5', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem', flexWrap: 'wrap' }}>
              <span style={{ fontSize: '13px', color: '#374151', fontFamily: 'DM Sans, sans-serif' }}>New to NMD Pressure Washing?</span>
              <Link href="/client/register" style={{ fontSize: '13px', fontWeight: 700, color: '#0F766E', textDecoration: 'none', fontFamily: 'DM Sans, sans-serif', whiteSpace: 'nowrap' }}>Create a Free Account &rarr;</Link>
            </div>
          )}

          <div className="flex flex-col gap-[8px] mt-[24px] pt-[20px] border-t border-[#E5E7EB] text-center">
            {(portalRole === 'admin' || portalRole === 'technical') && (
