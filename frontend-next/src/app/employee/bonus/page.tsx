  ...
  return (
    <PortalShell requiredRole="employee">
      <div style={{ marginBottom: "1.5rem" }}>
        ...
        <h1 style={{ fontFamily: "DM Sans, sans-serif", fontSize: "28px", fontWeight: 800, color: "#111827", letterSpacing: "-0.025em", marginBottom: 6 }}>My Bonus</h1>
        <p style={{ color: "#6B7280", fontSize: "14px", margin: 0 }}>Your 7-day rolling revenue and estimated bonus. Bonuses require admin approval before payout.</p>
      </div>
      {loading && <LoadingCard />}
      ...
      {data && (
        <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "1rem" }}>
            <div style={{ background: "white", border: "1px solid #E5E7EB", borderRadius: 10, padding: "1.25rem", textAlign: "center" }}>
              <div style={{ fontSize: "0.75rem", fontWeight: 600, color: "#9CA3AF", marginBottom: 6 }}>Weekly Revenue</div>
              <div style={{ fontSize: "1.6rem", fontWeight: 800, color: "#111827" }}>{money(data.weeklyRevenue)}</div>
            </div>
            <div style={{ background: "white", border: "1px solid #E5E7EB", borderRadius: 10, padding: "1.25rem", textAlign: "center" }}>
              <div style={{ fontSize: "0.75rem", fontWeight: 600, color: "#9CA3AF", marginBottom: 6 }}>Bonus Rate</div>
              <div style={{ fontSize: "1.6rem", fontWeight: 800, color: "#0F766E" }}>{data.bonusPct}%</div>
            </div>
            <div style={{ background: "#F0FDF9", border: "1px solid #A7F3D0", borderRadius: 10, padding: "1.25rem", textAlign: "center" }}>
              <div style={{ fontSize: "0.75rem", fontWeight: 600, color: "#0F766E", marginBottom: 6 }}>Estimated Bonus</div>
              <div style={{ fontSize: "1.6rem", fontWeight: 800, color: "#0F766E" }}>{money(data.bonusAmount)}</div>
              <div style={{ fontSize: "0.72rem", color: "#92400E", marginTop: 4, fontWeight: 600 }}>Pending approval</div>
            </div>
            <div style={{ background: "white", border: "1px solid #E5E7EB", borderRadius: 10, padding: "1.25rem", textAlign: "center" }}>
              <div style={{ fontSize: "0.75rem", fontWeight: 600, color: "#9CA3AF", marginBottom: 6 }}>Jobs Completed</div>
              <div style={{ fontSize: "1.6rem", fontWeight: 800, color: "#111827" }}>{data.jobsCompleted}</div>
            </div>
          </div>

          {data.toNextTier > 0 && data.nextTierPct && (
            <div style={{ background: "#FEF9C3", border: "1px solid #FDE68A", borderRadius: 10, padding: "1.25rem" }}>
              <div style={{ fontWeight: 700, fontSize: "0.9rem", color: "#92400E", marginBottom: 4 }}>Next Tier Unlock</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {tiers.map(t => (
                <div key={t.tier} style={{ display: "flex", alignItems: "center", gap: 12, padding: "0.75rem 1rem", borderRadius: 8, background: data.tier === t.tier ? "#F0FDF9" : "#F8FAF9", border: data.tier === t.tier ? "1px solid #A7F3D0" : "1px solid transparent" }}>
                  ...
                  <div style={{ fontSize: "1rem", fontWeight: 800, color: data.tier === t.tier ? "#0F766E" : "#9CA3AF" }}>{t.pct}</div>
                  {data.tier === t.tier && <span style={{ fontSize: "0.72rem", background: "#0F766E", color: "white", padding: "2px 8px", borderRadius: 20, fontWeight: 600 }}>Current</span>}
                </div>
              ))}
            </div>
          </div>
