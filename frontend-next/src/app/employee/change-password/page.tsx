...
export default function EmployeeChangePasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update stored auth to clear mustChangePassword
      if (auth?.user) {
        saveNmdAuth({ token: auth.token, user: { ...auth.user, mustChangePassword: false } })
      }

      setSuccess(true)
      setTimeout(() => router.replace('/dashboard/employee'), 2000)
    } catch (err) {
      ...
      <div style={{ background: 'white', borderRadius: 16, border: '1px solid #dde4ef', padding: '2.5rem', width: '100%', maxWidth: 420, boxShadow: '0 8px 40px rgba(14,17,23,0.07)' }}>
        ...
        {isMustChange && (
          <div style={{ background: '#fff9e6', border: '1px solid #f5e6a0', borderRadius: 8, padding: '0.75rem 1rem', marginBottom: '1.25rem', fontSize: '0.85rem', color: '#7a5c00', lineHeight: 1.5 }}>
            🔐 <strong>Action required:</strong> You must set a new password before accessing the portal.
          </div>
        )}
        ...
        {success && (
          <div style={{ background: '#f0fff4', border: '1px solid #c0dd97', borderRadius: 8, padding: '0.65rem 0.9rem', fontSize: '0.85rem', color: '#1f6132', fontWeight: 600, marginBottom: '1rem' }}>
            ✓ Password changed! Redirecting to your portal...
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <button type="submit" disabled={saving || success} style={{ width: '100%', padding: '0.75rem', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #1f6132, #124d83)', color: 'white', fontWeight: 700, fontSize: '0.95rem', cursor: saving || success ? 'not-allowed' : 'pointer', opacity: saving || success ? 0.7 : 1, marginTop: 4, fontFamily: 'DM Sans, sans-serif' }}>
        ...
        {!isMustChange && (
          <div style={{ marginTop: '1.25rem', textAlign: 'center', borderTop: '1px solid #dde4ef', paddingTop: '1.25rem' }}>
            <Link href="/dashboard/employee" style={{ fontSize: '0.85rem', color: '#5a6a88' }}>← Back to Dashboard</Link>
          </div>
        )}
      </div>
    </div>
