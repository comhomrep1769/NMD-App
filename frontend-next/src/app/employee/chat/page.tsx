    ...
    <PortalShell requiredRole="employee">
      <div style={{ marginBottom: "1.5rem", display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div>
          ...
          <h1 style={{ fontFamily: "DM Sans, sans-serif", fontSize: "28px", fontWeight: 800, color: "#111827", letterSpacing: "-0.025em", marginBottom: 6 }}>Chat</h1>
          <p style={{ color: "#6B7280", fontSize: "14px", margin: 0 }}>Communicate with your team and admin.</p>
        </div>
        <button
          onClick={() => setShowNewChat(!showNewChat)}
          style={{ padding: "0.6rem 1.1rem", borderRadius: 8, background: "#0F766E", color: "white", border: "none", fontWeight: 600, fontSize: "0.85rem", cursor: "pointer", fontFamily: "DM Sans, sans-serif", marginTop: 4 }}
        >
          + New Chat
        </button>
      </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {users.map(u => (
              <button
                ...
                style={{ padding: "0.4rem 0.9rem", borderRadius: 20, border: "1px solid #E5E7EB", background: "white", cursor: "pointer", fontSize: "0.82rem", fontFamily: "DM Sans, sans-serif", display: "flex", alignItems: "center", gap: 6 }}
              >
                <span style={{ width: 8, height: 8, borderRadius: "50%", background: ROLE_COLORS[u.role] || "#9CA3AF", display: "inline-block" }} />
                {u.displayName}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: "1rem", height: "65vh" }}>
          <div style={{ background: "white", border: "1px solid #E5E7EB", borderRadius: 10, overflow: "auto" }}>
            {conversations.length === 0 && (
              <div style={{ padding: "2rem", textAlign: "center", color: "#9CA3AF", fontSize: "0.85rem" }}>No conversations yet.</div>
            )}
            {conversations.map(c => (
              <div
                key={c.id}
                onClick={() => setActiveConv(c.id)}
                style={{ padding: "0.85rem 1rem", cursor: "pointer", borderBottom: "1px solid #E5E7EB", background: activeConv === c.id ? "#F0FDF9" : "white", borderLeft: activeConv === c.id ? "3px solid #0F766E" : "3px solid transparent" }}
              >
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                  {c.is_group && <span style={{ fontSize: "0.65rem", background: "#0F766E", color: "white", padding: "1px 6px", borderRadius: 10, fontWeight: 700 }}>GROUP</span>}
                  <div style={{ fontSize: "0.82rem", fontWeight: 600, color: "#111827" }}>{getConvLabel(c)}</div>
                </div>
                <div style={{ fontSize: "0.75rem", color: "#9CA3AF" }}>{c.last_message || "No messages yet"}</div>
              </div>
            ))}
          </div>

          <div style={{ background: "white", border: "1px solid #E5E7EB", borderRadius: 10, display: "flex", flexDirection: "column" }}>
            {!activeConv ? (
              ...
              <>
                ...
                <div style={{ padding: "0.85rem", borderTop: "1px solid #E5E7EB", display: "flex", gap: 8, alignItems: "center" }}>
                  <input ref={fileRef} type="file" accept="image/*" style={{ display: "none" }} onChange={e => handleImageUpload(e.target.files)} />
                  <button
                    onClick={() => fileRef.current?.click()}
                    style={{ padding: "0.55rem 0.75rem", borderRadius: 8, border: "1px solid #E5E7EB", background: "white", cursor: "pointer", fontSize: "1rem" }}
                    title="Send image"
                  >
                    📷
                  </button>
                  <input
                    ...
                    style={{ flex: 1, padding: "0.55rem 0.85rem", borderRadius: 8, border: "1.5px solid #E5E7EB", fontSize: "0.875rem", fontFamily: "DM Sans, sans-serif", outline: "none", color: "#111827" }}
                  />
                  <button
                    onClick={() => sendMessage()}
                    disabled={sending}
                    style={{ padding: "0.55rem 1.25rem", borderRadius: 8, background: "#0F766E", color: "white", border: "none", fontWeight: 600, cursor: "pointer", fontFamily: "DM Sans, sans-serif" }}
                  >
                    Send
                  </button>
                </div>
