...
export default function AdminTreatments() {
  ...
  return (
    <PortalShell requiredRole={["admin", "superadmin"]}>
      <SectionHeader
        title="Treatments"
        sub="Chemical treatments, dilution ratios, and safety notes."
      />
        ...
        <>
          <div style={{ display: "flex", gap: 10, marginBottom: "1.5rem", flexWrap: "wrap" }}>
            <input
              ...
              style={{ padding: "0.55rem 0.85rem", borderRadius: 8, border: "1px solid #E5E7EB", fontSize: "0.875rem", fontFamily: "DM Sans, sans-serif", outline: "none", minWidth: 200, color: "#111827", background: "white" }}
            />
            <select
              value={category}
              onChange={e => setCategory(e.target.value)}
              style={{ padding: "0.55rem 0.85rem", borderRadius: 8, border: "1px solid #E5E7EB", fontSize: "0.875rem", fontFamily: "DM Sans, sans-serif", outline: "none", color: "#111827", background: "white" }}
            >
              {categories.map(c => <option key={c}>{c}</option>)}
            </select>
            <div style={{ marginLeft: "auto", fontSize: "0.85rem", color: "#9CA3AF", display: "flex", alignItems: "center" }}>{filtered.length} treatments</div>
          </div>
          {filtered.length === 0 ? (
