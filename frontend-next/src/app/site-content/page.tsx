...
const FIELD_HINTS: Record<string, string> = {
  'seo.global.search_console_verification':
    "Paste only the verification code Google gives you — e.g. from <meta name=\"google-site-verification\" content=\"THIS PART\" />, not the whole tag.",
}
...
export default function SiteContentPage() {
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <button
          ...
          style={{
            ...
            background: !dirty || isSaving ? '#E5E7EB' : '#0F766E',
            color: !dirty || isSaving ? '#9CA3AF' : 'white',
  ...
  // ── Reusable field renderer for text / richtext / image, shared by
  // the Content, Images, and SEO tabs so all three stay visually consistent ──
  const FieldCard = ({ item, imageStyle }: { item: ContentItem; imageStyle?: boolean }) => {
    const draft = drafts[item.key] ?? ''
    const hint = FIELD_HINTS[item.key]
    return (
      <div style={{ background: 'white', border: '1px solid #E5E7EB', borderRadius: 10, padding: imageStyle ? '1rem' : '1rem 1.25rem', display: imageStyle ? 'flex' : undefined, flexDirection: imageStyle ? 'column' : undefined }}>
        ...
        {hint && (
          <div style={{ fontSize: '0.75rem', color: '#9CA3AF', marginBottom: 6, lineHeight: 1.4 }}>{hint}</div>
        )}

        {item.valueType === 'image' ? (
          <>
            {draft && (
              <img src={draft} alt={item.label}
                style={{ width: '100%', height: imageStyle ? 160 : 180, objectFit: 'cover', borderRadius: 8, border: '1px solid #E5E7EB', marginBottom: 8 }}
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <input
                ...
                style={{ flex: 1, padding: '0.5rem 0.75rem', borderRadius: 8, border: '1.5px solid #E5E7EB', fontSize: '0.78rem', fontFamily: 'DM Sans, sans-serif', color: '#111827', outline: 'none' }}
              />
              <input
                ref={el => { fileRefs.current[item.key] = el }}
                type="file" accept="image/*" style={{ display: 'none' }}
                onChange={e => { handleImageUpload(item.key, e.target.files); e.target.value = '' }}
              />
              <button
                onClick={() => fileRefs.current[item.key]?.click()}
                style={{ padding: '0.5rem 0.9rem', borderRadius: 8, border: '1px solid #E5E7EB', background: 'white', color: '#374151', fontWeight: 600, fontSize: '0.78rem', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', whiteSpace: 'nowrap' }}
              >
                Upload
              </button>
            </div>
  ...
  return (
    <PortalShell requiredRole={['admin', 'superadmin']}>
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <p style={{ color: '#6B7280', fontSize: '14px', margin: 0 }}>Edit titles, text, images, and SEO settings shown on the public website.</p>
      </div>
        ...
        <>
          ...
          {tab === 'content' && contentPages.map(page => (
            <div key={page} style={{ marginBottom: '2rem' }}>
              <div style={{ fontWeight: 700, fontSize: '0.95rem', color: '#111827', marginBottom: '1rem' }}>
                {PAGE_LABELS[page] || page}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                {contentItems.filter(i => i.page === page).map(item => (
                  <FieldCard key={item.key} item={item} />
                ))}
              </div>
            </div>
          ))}
          {tab === 'content' && contentPages.length === 0 && (
            <div style={{ background: 'white', border: '1px solid #E5E7EB', borderRadius: 10, padding: '2.5rem', textAlign: 'center', color: '#9CA3AF', fontSize: '0.85rem' }}>
              No text content fields yet.
            </div>
          )}

          {/* ── Images tab — every image field across every page, gallery style ── */}
          {tab === 'images' && imagePages.map(page => (
            <div key={page} style={{ marginBottom: '2rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1rem' }}>
                {imageItems.filter(i => i.page === page).map(item => (
                  <FieldCard key={item.key} item={item} imageStyle />
                ))}
              </div>
            </div>
          ))}
          {tab === 'images' && imagePages.length === 0 && (
            <div style={{ background: 'white', border: '1px solid #E5E7EB', borderRadius: 10, padding: '2.5rem', textAlign: 'center', color: '#9CA3AF', fontSize: '0.85rem' }}>
              No image fields yet. As more pages get wired into Site Content, their images will automatically show up here.
            </div>
          )}
          ...
          {tab === 'seo' && seoPages.map(page => (
            <div key={page} style={{ marginBottom: '2rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                {seoItems.filter(i => i.page === page).map(item => (
                  <FieldCard key={item.key} item={item} />
                ))}
              </div>
            </div>
          ...
          {tab === 'seo' && seoPages.length === 0 && (
            <div style={{ background: 'white', border: '1px solid #E5E7EB', borderRadius: 10, padding: '2.5rem', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '0.85rem', color: '#6B7280', maxWidth: 420, margin: '0 auto', lineHeight: 1.6 }}>
                As more pages get wired into Site Content, their title, meta description, and social share image will appear here.
              </div>
            </div>
          )}
        </>
