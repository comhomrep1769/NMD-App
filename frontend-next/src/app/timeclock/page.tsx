  ...
  sessions.forEach(s => {
    ...
    if (!employeeMap.has(key)) {
      employeeMap.set(key, {
        ...
        sessions: [], totalPaidMins: 0, totalSessions: 0, activeSessions: 0
      })
    }
    ...
    <PortalShell requiredRole={["admin", "superadmin"]}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.5rem', flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ color: "#5a6a88", fontSize: "0.875rem" }}>All employee hours — grouped by employee. Connected to payroll.</p>
        </div>
              ...
              {employees.map(emp => {
                ...
                return (
                  <div key={emp.userId} style={{ background: 'white', border: `1.5px solid ${hasActive ? 'rgba(230,126,34,0.4)' : '#dde4ef'}`, borderRadius: 14, overflow: 'hidden' }}>
                    {/* Employee header row */}
                    <div
                      onClick={() => setExpandedEmployee(isExpanded ? null : emp.userId)}
