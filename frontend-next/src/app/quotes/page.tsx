  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      {!selectedClient && value && (
        <div style={{ marginTop: 4, fontSize: '0.75rem', color: '#92400E' }}>⚠ No client selected — quote can be saved as draft only</div>
      )}
...
export default function QuotesPage() {
  ...
  const filtered = quotes
    ...
    .filter(q => `${q.quoteNumber} ${q.clientName} ${q.serviceType} ${q.status}`.toLowerCase().includes(search.toLowerCase()))
    ...
    <PortalShell requiredRole={['admin', 'superadmin']}>
        ...
        <div style={modalOverlay}>
          <div style={modalBox}>
            ...
            <form onSubmit={handleCreate} style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {formError && <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, padding: '0.65rem 1rem', fontSize: '0.82rem', color: '#B91C1C' }}>{formError}</div>}
              <div>
              ...
              {form.status === 'sent' && !createSelectedClient && (
                <div style={{ background: '#FEF9C3', border: '1px solid #FDE68A', borderRadius: 8, padding: '0.65rem 1rem', fontSize: '0.82rem', color: '#92400E' }}>
                  You must select a registered client to send a quote — we need their email on file.
                </div>
              )}
              <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
                <button type="button" onClick={() => { setShowCreate(false); setFormError('') }} style={{ flex: 1, padding: '0.7rem', borderRadius: 8, border: '1px solid #E5E7EB', background: 'white', color: '#6B7280', fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Cancel</button>
                <button type="submit" disabled={saving} style={{ flex: 2, padding: '0.7rem', borderRadius: 8, border: 'none', background: '#0F766E', color: 'white', fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer', fontFamily: 'DM Sans, sans-serif', opacity: saving ? 0.7 : 1 }}>
      ...
      {sendQuote && (
        <div style={modalOverlay}>
          <div style={modalBox}>
            <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid #E5E7EB', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ fontFamily: 'DM Sans, sans-serif', fontWeight: 700, fontSize: '1rem', color: '#111827' }}>Send Quote to Client</div>
            ...
            <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {sendError && <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, padding: '0.65rem 1rem', fontSize: '0.82rem', color: '#B91C1C' }}>{sendError}</div>}
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => setSendQuote(null)} style={{ flex: 1, padding: '0.7rem', borderRadius: 8, border: '1px solid #E5E7EB', background: 'white', color: '#6B7280', fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Cancel</button>
                <button
        ...
        <div style={modalOverlay}>
          <div style={{ ...modalBox, maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem', overflowY: 'auto' }}>
              {convertError && <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, padding: '0.65rem 1rem', fontSize: '0.82rem', color: '#B91C1C' }}>{convertError}</div>}
              <div>
                <label style={labelStyle}>Upload Invoice from Bank App (Optional)</label>
                <p style={{ fontSize: '0.78rem', color: '#9CA3AF', marginBottom: 8, lineHeight: 1.5 }}>
                ...
                <button type="button" onClick={() => fileRef.current?.click()}
                  style={{ width: '100%', padding: '0.75rem', borderRadius: 8, border: '1px dashed #D1D5DB', background: uploadFile ? '#F0FDF9' : '#F9FAFB', color: uploadFile ? '#059669' : '#374151', fontWeight: 500, fontSize: '0.875rem', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
                  {uploadFile ? `Selected: ${uploadFile.name}` : '+ Select Invoice File (PDF or Image)'}
                </button>
                {uploadFile && (
                  <button type="button" onClick={() => setUploadFile(null)}
                    style={{ marginTop: 6, background: 'none', border: 'none', color: '#DC2626', fontSize: '0.78rem', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
                    Remove file
                  </button>
                )}
              </div>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => { setConvertQuote(null); setUploadFile(null); setConvertError('') }}
                  style={{ flex: 1, padding: '0.7rem', borderRadius: 8, border: '1px solid #E5E7EB', background: 'white', color: '#6B7280', fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Cancel</button>
                <button onClick={handleConvertWithUpload} disabled={converting}
                  style={{ flex: 2, padding: '0.7rem', borderRadius: 8, border: 'none', background: '#0F766E', color: 'white', fontWeight: 600, cursor: converting ? 'not-allowed' : 'pointer', fontFamily: 'DM Sans, sans-serif', opacity: converting ? 0.7 : 1 }}>
      ...
      {deleteQuote && (
        <div style={modalOverlay}>
          <div style={{ ...modalBox, maxWidth: 420 }}>
            <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid #E5E7EB', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ fontFamily: 'DM Sans, sans-serif', fontWeight: 700, fontSize: '1rem', color: '#111827' }}>Delete Quote</div>
              <button onClick={() => setDeleteQuote(null)} style={{ background: 'none', border: 'none', fontSize: '1.25rem', cursor: 'pointer', color: '#9CA3AF' }}>×</button>
            </div>
            <div style={{ padding: '1.5rem' }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => setDeleteQuote(null)} style={{ flex: 1, padding: '0.7rem', borderRadius: 8, border: '1px solid #E5E7EB', background: 'white', color: '#6B7280', fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Cancel</button>
                <button onClick={handleDelete} disabled={deleting}
                  style={{ flex: 1, padding: '0.7rem', borderRadius: 8, border: 'none', background: '#DC2626', color: 'white', fontWeight: 600, cursor: deleting ? 'not-allowed' : 'pointer', fontFamily: 'DM Sans, sans-serif', opacity: deleting ? 0.7 : 1 }}>
      ...
      {!loading && !error && (
        <DataTable
          ...
          rows={filtered.map(q => [
            <span key="num" style={{ fontWeight: 700, color: '#0F766E' }}>#{q.quoteNumber}</span>,
            <span key="client" style={{ fontWeight: 500 }}>
              {q.clientName || '—'}
              {!q.clientId && q.status === 'draft' && (
                <span style={{ fontSize: '0.7rem', color: '#92400E', marginLeft: 6, fontWeight: 500 }}>· no client linked</span>
              )}
            </span>,
            <span key="svc" style={{ color: '#6B7280' }}>{q.serviceType || '—'}</span>,
            ...
            <div key="actions" style={{ display: 'flex', gap: 5, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              {q.status === 'sent' && (
                <span style={{ fontSize: '0.75rem', color: '#9CA3AF', fontWeight: 600 }}>⏳ Awaiting client response</span>
              )}
              ...
              {q.convertedInvoiceId && (
                <span style={{ fontSize: '0.75rem', color: '#059669', fontWeight: 600 }}>Invoiced</span>
              )}
