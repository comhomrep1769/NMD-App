...
async function getJobClientInfo(jobId: number) {
...
async function getEmployeeName(userId: number) {
  const result = await pool.query(
...
// POST /api/sms/on-way
router.post('/on-way', requireAuth, async (req: Request, res: Response) => {
  try {
    const { jobId, eta } = req.body as { jobId?: number; eta?: string };
    ...
    const result = await sendEmployeeOnWaySms(
      job.client_phone,
      job.client_name,
      employeeName,
      eta
...
router.post('/arrived', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const employeeName = await getEmployeeName(Number(userId));
    const result = await sendEmployeeArrivedSms(
      job.client_phone,
      job.client_name,
      employeeName
    );
...
router.post('/job-complete', requireAuth, async (req: Request, res: Response) => {
  try {
    const { jobId } = req.body as { jobId?: number };
