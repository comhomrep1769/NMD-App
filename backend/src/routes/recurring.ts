...
async function sendRecurringReminder(service: any) {
  ...
  await pool.query(
    `UPDATE recurring_services SET last_reminder_sent_at = NOW() WHERE id = $1`,
    [service.id]
  );
...
router.get("/", requireAuth, requireRole("admin"), async (_req, res) => {
  try {
    const result = await pool.query(
      `SELECT * FROM recurring_services ORDER BY next_service_date ASC NULLS LAST, created_at DESC`
    );
...
router.post("/client-optin", requireAuth, requireRole("client"), async (req, res) => {
  try {
    ...
    const { invoiceId, frequency, serviceType } = req.body as {
      invoiceId?: string;
      frequency?: string;
      serviceType?: string;
    };
    ...
    const clientResult = await pool.query(
      `SELECT * FROM clients WHERE user_id = $1 OR LOWER(email) = LOWER($2) ORDER BY created_at DESC LIMIT 1`,
      [userId, userEmail]
    );
    ...
    const invoiceResult = await pool.query(
      `SELECT * FROM invoices WHERE id = $1 AND (client_id = $2 OR LOWER(client_name) = LOWER($3)) LIMIT 1`,
      [invoiceId, client.id, `${client.first_name} ${client.last_name}`]
    );
    ...
    const existing = await pool.query(
      `SELECT id FROM recurring_services WHERE client_id = $1 AND LOWER(service_type) = LOWER($2) AND status = 'active' LIMIT 1`,
      [client.id, serviceType || invoice.job_name]
    );
...
router.post("/", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    const {
      clientId, clientName, phone, email, address,
      serviceType, frequency, price, status, nextServiceDate, notes
    } = req.body;
...
router.patch("/:id", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [id, clientId || null, clientName ?? null, phone ?? null, email ?? null,
       address ?? null, serviceType ?? null, frequency ?? null, price ?? null,
       status ?? null, nextServiceDate || null, notes ?? null]
    );

    if (result.rows.length === 0) return res.status(404).json({ error: "Recurring service not found" });
...
router.patch("/:id/status", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE recurring_services SET status = $2 WHERE id = $1 RETURNING *`,
      [id, status]
    );
...
router.post("/:id/create-stripe-subscription", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    ...
    if (!service.email) return res.status(400).json({ error: "Client email is required before creating a Stripe subscription." });
    if (Number(service.price || 0) <= 0) return res.status(400).json({ error: "Recurring service price must be greater than 0." });
    ...
    const updated = await pool.query(
      `UPDATE recurring_services SET stripe_checkout_session_id = $2, stripe_checkout_url = $3, stripe_payment_status = 'checkout_created' WHERE id = $1 RETURNING *`,
...
router.post("/:id/send-reminder", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    ...
    if (!reminder.sent) return res.status(400).json({ error: reminder.reason || "Reminder not sent" });
...
router.post("/scan-reminders", requireAuth, requireRole("admin"), async (_req, res) => {
  try {
    ...
    for (const service of result.rows) {
      ...
      if (reminder.sent) { sent += 1; }
      else { skipped.push({ id: service.id, clientName: service.client_name, reason: reminder.reason || "Reminder not sent" }); }
    }
...
router.post("/:id/create-next-job", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    ...
    if (recurringResult.rows.length === 0) return res.status(404).json({ error: "Recurring service not found" });

    const service = recurringResult.rows[0];
    if (service.status !== "active") return res.status(400).json({ error: "Only active recurring services can create jobs" });
    ...
    const jobResult = await pool.query(
      ...
      [service.service_type, service.client_name, service.address,
