...
router.post("/", requireAuth, requireRole("admin", "superadmin"), async (req, res) => {
  try {
    const { firstName, lastName, phone, email, address, createLogin } = req.body as {
      firstName?: string; lastName?: string; phone?: string; email?: string;
      address?: string; createLogin?: boolean;
    };
    ...
    if (createLogin) {
      ...
      const userResult = await pool.query(
        `INSERT INTO users (email, password_hash, display_name, role, pay_rate, phone, address, must_change_password)
        VALUES ($1, $2, $3, 'client', NULL, $4, $5, TRUE)
        RETURNING id`,
        [email.trim().toLowerCase(), passwordHash, displayName, phone ?? null, address ?? null]
      );
...
router.delete("/:clientId", requireAuth, requireRole("admin", "superadmin"), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `DELETE FROM clients WHERE id = $1 RETURNING id, user_id`,
      [clientId]
    );
