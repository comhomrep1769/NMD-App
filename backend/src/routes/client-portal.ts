...
router.post("/service-request", requireAuth, requireRole("client"), async (req, res) => {
  try {
    ...
    const {
      address, serviceType, preferredDate, preferredTime, notes,
      photoDataUrl, photoNote, waiverAccepted, waiverSignature
    } = req.body as {
      address?: string; serviceType?: string; preferredDate?: string | null;
