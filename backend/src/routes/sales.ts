...
router.get("/me", requireAuth, async (req, res) => {
  try {
    ...
    const [commissionsResult, invoicesResult] = await Promise.all([
      pool.query(
        `SELECT sc.*, u.display_name AS sales_rep_name,
      ...
      pool.query(
        `SELECT id, invoice_number, client_name, job_name, total, status, created_at
         FROM invoices WHERE sales_rep_id = $1 ORDER BY created_at DESC`,
        [userId]
      )
    ]);
...
router.get("/commissions", requireAuth, requireRole("admin"), async (_req, res) => {
  try {
    const result = await pool.query(
      `SELECT sc.*, u.display_name AS sales_rep_name, i.invoice_number, i.client_name
       FROM sales_commissions sc
       JOIN users u ON u.id = sc.sales_rep_id
       JOIN invoices i ON i.id = sc.invoice_id
       ORDER BY sc.created_at DESC`
...
router.post("/commissions", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    const { salesRepId, invoiceId, commissionType, rate, subtotal, tier, notes } = req.body as {
      salesRepId?: string; invoiceId?: string; commissionType?: string;
      rate?: number; subtotal?: number; tier?: number; notes?: string;
    };
    ...
    const result = await pool.query(
      `INSERT INTO sales_commissions (sales_rep_id, invoice_id, commission_type, rate, subtotal, amount, tier, notes)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       RETURNING *`,
      [salesRepId, invoiceId, commissionType || 'one_time', rate, subtotal, amount, tier || 1, notes || null]
    );
...
router.patch("/commissions/:id", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    ...
    const { status, notes } = req.body as {
      status?: "pending" | "approved" | "paid"; notes?: string;
    };
