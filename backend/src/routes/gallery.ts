...
function publicRow(row: any, base: string) {
  return {
    id: row.id,
    title: row.title,
...
router.get("/", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT id, title, service_category, city, state, description, job_date,
...
router.get("/:id/image/:slot", async (req: any, res: any) => {
  try {
    await ensureGalleryTable();
    ...
    const result = await pool.query(
      `SELECT ${column} AS value FROM gallery_jobs WHERE id = $1 AND is_published = true LIMIT 1`,
      [id]
    );
    ...
    if (value.startsWith("data:")) {
      ...
      if (semiIdx > 5) {
        ...
        if (!mimeType.startsWith("image/")) return res.status(415).json({ error: "Unsupported type" });
    ...
    if (value.startsWith("http") || value.startsWith("/")) return res.redirect(value);
...
router.post("/", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    await ensureGalleryTable();
    const {
      title, serviceCategory, city, state, description,
      beforeImage, afterImage, jobDate, isPublished, displayOrder,
    } = req.body as Record<string, any>;

    if (!title?.trim()) return res.status(400).json({ error: "Title is required." });
    if (!city?.trim()) return res.status(400).json({ error: "City is required." });
    if (!isUsableImage(beforeImage)) return res.status(400).json({ error: "A before photo is required." });
    if (!isUsableImage(afterImage)) return res.status(400).json({ error: "An after photo is required." });
...
router.patch("/:id", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    ...
    const setIf = (column: string, value: any) => {
    ...
    if (typeof body.serviceCategory === "string") setIf("service_category", body.serviceCategory);
    if (typeof body.city === "string") {
      if (!body.city.trim()) return res.status(400).json({ error: "City cannot be empty." });
      setIf("city", body.city.trim());
    }
    if (typeof body.state === "string") setIf("state", body.state.trim() || "FL");
    if (typeof body.description === "string") setIf("description", body.description.trim() || null);
    if (body.jobDate !== undefined) setIf("job_date", body.jobDate || null);
    if (typeof body.isPublished === "boolean") setIf("is_published", body.isPublished);
    if (body.displayOrder !== undefined) setIf("display_order", Number(body.displayOrder) || 0);

    for (const [key, column] of [["beforeImage", "before_image"], ["afterImage", "after_image"]] as const) {
      if (body[key] !== undefined) {
        if (!isUsableImage(body[key])) return res.status(400).json({ error: `Invalid ${key}.` });
        if (body[key].length > MAX_IMAGE_CHARS) return res.status(400).json({ error: "Photo too large. Max 10MB each." });
    ...
    values.push(id);
    const result = await pool.query(
      `UPDATE gallery_jobs SET ${fields.join(", ")} WHERE id = $${n} RETURNING id`,
      values
    );
