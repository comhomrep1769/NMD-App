...
router.post("/", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    const { clientId, clientName, serviceType, total, status } = req.body as {
      clientId?: string | null; clientName?: string; serviceType?: string;
      total?: number; status?: "draft" | "sent" | "accepted" | "declined" | "expired";
    };
    ...
    const result = await pool.query(
      `INSERT INTO quotes (client_id, client_name, service_type, total, status, accepted_at)
      VALUES ($1, $2, $3, $4, $5, CASE WHEN $5 = 'accepted' THEN NOW() ELSE NULL END)
      RETURNING id, quote_number, client_id, client_name, service_type, total, status, converted_invoice_id, accepted_at, created_at`,
...
router.patch("/:quoteId", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    ...
    if (beforeResult.rows.length === 0) return res.status(404).json({ error: "Quote not found" });
...
router.post("/:quoteId/client-accept", requireAuth, requireRole("client"), async (req, res) => {
  try {
    ...
    const clientCheck = await pool.query(
      `SELECT * FROM clients WHERE LOWER(email) = LOWER($1) LIMIT 1`, [userEmail]
    );
...
router.post("/:quoteId/client-decline", requireAuth, requireRole("client"), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE quotes SET status = 'declined' WHERE id = $1
      RETURNING id, quote_number, client_id, client_name, service_type, total, status, converted_invoice_id, accepted_at, created_at`,
      [quoteId]
    );
...
router.post("/:quoteId/convert-to-invoice", requireAuth, requireRole("admin"), async (req, res) => {
  ...
  try {
    ...
    if (quoteResult.rows.length === 0) { await client.query("ROLLBACK"); return res.status(404).json({ error: "Quote not found" }); }

    const quote = quoteResult.rows[0];
    if (quote.converted_invoice_id) { await client.query("ROLLBACK"); return res.status(400).json({ error: "This quote has already been converted to an invoice." }); }
    if (quote.status !== "accepted") { await client.query("ROLLBACK"); return res.status(400).json({ error: "Only accepted quotes can be converted to invoices." }); }

    const invoiceResult = await client.query(
      `INSERT INTO invoices (client_id, client_name, job_name, total, status)
      VALUES ($1, $2, $3, $4, 'unpaid')
      RETURNING id, invoice_number, client_id, client_name, job_name, total, status, job_id, assigned_user_id, created_at, payment_provider, payment_link_id, payment_link_url, payment_status, payment_created_at, stripe_checkout_session_id`,
      [quote.client_id || null, quote.client_name, quote.service_type, quote.total || 0]
    );
