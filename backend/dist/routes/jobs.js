...
router.post("/check-conflicts", requireAuth, requireRole("admin"), async (req, res) => {
    try {
        const { startTime, endTime, assignedUserIds } = req.body;
...
router.post("/", requireAuth, requireRole("admin"), async (req, res) => {
    try {
        const { title, clientName, address, startTime, endTime, status, notes, assignedUserIds, forceCreate } = req.body;
...
router.patch("/:jobId", requireAuth, requireRole("admin"), async (req, res) => {
    try {
        ...
        const { title, clientName, address, startTime, endTime, status, notes, assignedUserIds, forceUpdate } = req.body;
        const currentJobResult = await pool.query(`SELECT start_time, end_time FROM jobs WHERE id = $1 LIMIT 1`, [jobId]);
        ...
        `, [
            ...
            if (status === 'completed') {
                try {
                    const invResult = await pool.query(`SELECT invoice_number, client_name, client_id, job_name, total, payment_link_url
               FROM invoices
               WHERE job_id = $1 AND status = 'unpaid' AND payment_link_url IS NOT NULL
               LIMIT 1`, [jobId]);
                    if (invResult.rows.length > 0) {
                        const inv = invResult.rows[0];
                        const clientEmailResult = await pool.query(`SELECT email FROM clients WHERE id = $1 OR LOWER(CONCAT(first_name,' ',last_name)) = LOWER($2) LIMIT 1`, [inv.client_id || 'none', inv.client_name]);
...
router.post("/:jobId/claim", requireAuth, requireRole("employee"), async (req, res) => {
    try {
        ...
        const jobResult = await pool.query(`SELECT * FROM jobs WHERE id = $1 AND status = 'scheduled' LIMIT 1`, [jobId]);
        if (jobResult.rows.length === 0) {
            return res.status(404).json({ error: "Job not found or no longer available." });
        }
        await pool.query(`INSERT INTO job_assignments (job_id, user_id) VALUES ($1, $2) ON CONFLICT (job_id, user_id) DO NOTHING`, [jobId, userId]);
