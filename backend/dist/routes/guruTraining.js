...
router.post('/', requireAuth, requireRole("admin", "superadmin"), async (req, res) => {
    try {
        ...
        const result = await pool.query('INSERT INTO guru_training (category, title, content) VALUES ($1, $2, $3) RETURNING *', [category, title, content]);
...
router.put('/:id', requireAuth, requireRole("admin", "superadmin"), async (req, res) => {
    try {
       ...
       SET category = COALESCE($1, category),
           ...
           updated_at = NOW()
       WHERE id = $4
       RETURNING *`, [category, title, content, req.params.id]);
        if (result.rows.length === 0)
            return res.status(404).json({ error: 'Not found' });
        res.json(result.rows[0]);
    }
