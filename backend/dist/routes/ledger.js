...
router.get("/me", requireAuth, requireRole("employee", "admin"), async (req, res) => {
    try {
      ...
      `, [req.user.id]);
        ...
        const pendingCash = payments.filter((payment) => payment.paymentMethod === "cash" && payment.status === "pending_admin_approval");
        const approvedCash = payments.filter((payment) => payment.paymentMethod === "cash" &&
            (payment.status === "approved" || payment.status === "paid"));
        const rejectedCash = payments.filter((payment) => payment.paymentMethod === "cash" && payment.status === "rejected");
        const cardRecords = payments.filter((payment) => payment.paymentMethod === "card_link");
        const approvedPayments = payments.filter((payment) => payment.status === "approved" || payment.status === "paid");
        const summary = {
            ...
            approvedCollectedTotal: approvedPayments.reduce((sum, payment) => sum + payment.totalCollected, 0),
            approvedSubtotalTotal: approvedPayments.reduce((sum, payment) => sum + payment.amount, 0),
            approvedSalesTaxTotal: approvedPayments.reduce((sum, payment) => sum + payment.salesTaxAmount, 0),
