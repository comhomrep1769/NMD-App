...
async function ensureCompanyWideGroup() {
    ...
    WHERE c.is_group = TRUE
      AND c.group_name = 'Company-Wide'
      AND u.role IN ('admin', 'superadmin', 'employee')
      AND NOT EXISTS (
...
router.get("/users", requireAuth, async (req, res) => {
    try {
        ...
        if (role === "admin" || role === "superadmin") {
            result = await pool.query(`SELECT id, email, display_name, role
         FROM users
         WHERE id <> $1
         ORDER BY role ASC, display_name ASC`, [req.user.id]);
        }
        else if (role === "employee") {
            result = await pool.query(`SELECT id, email, display_name, role
         FROM users
         WHERE id <> $1
           AND role IN ('admin', 'superadmin', 'employee')
         ORDER BY role ASC, display_name ASC`, [req.user.id]);
        }
        else {
            result = await pool.query(`SELECT id, email, display_name, role
...
router.post("/conversations", requireAuth, async (req, res) => {
    try {
        ...
        const targetCheck = await pool.query(`SELECT id, role FROM users WHERE id = $1 LIMIT 1`, [targetUserId]);
        ...
        if (role === "employee" && targetRole === "client") {
      ...
      `, [req.user.id, targetUserId]);
        ...
        try {
            ...
            const conversation = await client.query(`INSERT INTO conversations (is_group) VALUES (FALSE) RETURNING id`);
            const conversationId = conversation.rows[0].id;
            await client.query(`INSERT INTO conversation_members (conversation_id, user_id)
         VALUES ($1, $2), ($1, $3)`, [conversationId, req.user.id, targetUserId]);
...
router.get("/conversations/:conversationId/messages", requireAuth, async (req, res) => {
    try {
        ...
        const memberCheck = await pool.query(`SELECT 1 FROM conversation_members
       WHERE conversation_id = $1 AND user_id = $2 LIMIT 1`, [conversationId, req.user.id]);
...
router.post("/conversations/:conversationId/messages", requireAuth, async (req, res) => {
    try {
      ...
      `, [conversationId, req.user.id, cleanBody, imageUrl || null]);
        const members = await pool.query(`SELECT user_id FROM conversation_members
       WHERE conversation_id = $1 AND user_id <> $2`, [conversationId, req.user.id]);
        const pushBody = cleanBody || "📷 Image";
