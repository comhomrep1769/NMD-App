...
async function geocodeAddress(address) {
    try {
        ...
        const res = await fetch(`https://nominatim.openstreetmap.org/search?q=${encoded}&format=json&limit=1`, { headers: { 'User-Agent': 'NMD-Pressure-Washing-App/1.0' } });
...
async function getJobClientInfo(jobId) {
    const result = await pool.query(`SELECT j.id, j.title AS service_name, c.name AS client_name,
...
async function getEmployeeName(userId) {
    const result = await pool.query(`SELECT display_name AS name FROM users WHERE id = $1`, [userId]);
    return result.rows[0]?.name || 'Your technician';
}
// ── ADMIN: Get all jobs for a date ───────────────────────────────────────────
// GET /api/routes/jobs-for-date?date=YYYY-MM-DD
router.get('/jobs-for-date', requireAuth, requireRole('admin'), async (req, res) => {
    try {
        const { date } = req.query;
        if (!date)
            return res.status(400).json({ error: 'date required' });
        const result = await pool.query(`SELECT j.id, j.title, j.client_name, j.address, j.start_time, j.end_time,
        ...
        for (const job of jobs) {
            if (!job.lat || !job.lng) {
                ...
                if (coords) {
                    job.lat = coords.lat;
                    job.lng = coords.lng;
                    await pool.query(`UPDATE jobs SET lat=$1, lng=$2 WHERE id=$3`, [coords.lat, coords.lng, job.id]);
                }
            }
        }
...
router.get('/employees', requireAuth, requireRole('admin'), async (_req, res) => {
    try {
        const result = await pool.query(`SELECT id, display_name AS name, email
       FROM users
       WHERE role = 'employee'
...
router.get('/my-route', requireAuth, async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT r.id AS route_id,
              rs.id AS stop_id,
              rs.stop_order,
              rs.departed_at,
              rs.arrived_at,
       ...
       ORDER BY rs.stop_order ASC`, [userId, routeDate]);
...
router.get('/', requireAuth, requireRole('admin'), async (req, res) => {
    try {
        ...
        if (!date)
            return res.status(400).json({ error: 'date required' });
        const result = await pool.query(`SELECT r.id, r.employee_id, r.route_date,
       ...
       ORDER BY u.display_name ASC`, [date]);
...
router.post('/', requireAuth, requireRole('admin'), async (req, res) => {
    try {
        const { employeeId, date, jobIds } = req.body;
        ...
        try {
            ...
            const routeResult = await client.query(`INSERT INTO routes (employee_id, route_date, created_by)
         VALUES ($1, $2, $3)
         ON CONFLICT (employee_id, route_date)
         DO UPDATE SET created_by = EXCLUDED.created_by
         RETURNING id`, [employeeId, date, createdBy]);
            const routeId = routeResult.rows[0].id;
            await client.query(`DELETE FROM route_stops WHERE route_id = $1`, [routeId]);
            for (let i = 0; i < jobIds.length; i++) {
                await client.query(`INSERT INTO route_stops (route_id, job_id, stop_order) VALUES ($1, $2, $3)`, [routeId, jobIds[i], i + 1]);
            }
...
router.put('/:routeId/stops', requireAuth, requireRole('admin'), async (req, res) => {
    try {
...
router.post('/stops/:stopId/depart', requireAuth, async (req, res) => {
    try {
        const { stopId } = req.params;
        const userId = req.user?.id;
        const stopResult = await pool.query(`UPDATE route_stops SET departed_at = NOW()
       WHERE id = $1
       RETURNING job_id, departed_at`, [stopId]);
        if (stopResult.rows.length === 0)
            return res.status(404).json({ error: 'Stop not found' });
        const jobId = stopResult.rows[0].job_id;
...
router.post('/stops/:stopId/arrive', requireAuth, async (req, res) => {
    try {
        const { stopId } = req.params;
        const userId = req.user?.id;
        const stopResult = await pool.query(`UPDATE route_stops SET arrived_at = NOW()
       WHERE id = $1
       RETURNING job_id, arrived_at`, [stopId]);
        if (stopResult.rows.length === 0)
            return res.status(404).json({ error: 'Stop not found' });
        const jobId = stopResult.rows[0].job_id;
        const job = await getJobClientInfo(jobId);
...
router.post('/stops/:stopId/complete', requireAuth, async (req, res) => {
    try {
        const { stopId } = req.params;
        const stopResult = await pool.query(`UPDATE route_stops SET completed_at = NOW()
       WHERE id = $1
       RETURNING job_id, completed_at`, [stopId]);
        if (stopResult.rows.length === 0)
            return res.status(404).json({ error: 'Stop not found' });
        const jobId = stopResult.rows[0].job_id;
        const job = await getJobClientInfo(jobId);
