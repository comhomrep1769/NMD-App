...
router.get("/", requireAuth, requireRole("admin"), async (_req, res) => {
    try {
        const result = await pool.query(`SELECT id, email, display_name, role, created_at, pay_rate, must_change_password
...
router.post("/:employeeId/reset-password", requireAuth, requireRole("admin"), async (req, res) => {
    try {
        const { employeeId } = req.params;
        const userResult = await pool.query(`SELECT id, email, display_name FROM users WHERE id = $1 AND role IN ('admin','employee') LIMIT 1`, [employeeId]);
        if (userResult.rows.length === 0) {
            return res.status(404).json({ error: "Employee not found" });
        }
