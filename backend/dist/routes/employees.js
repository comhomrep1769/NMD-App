...
router.get("/", requireAuth, requireRole("admin"), async (_req, res) => {
    try {
        const result = await pool.query(`SELECT id, email, display_name, role, created_at, pay_rate
...
router.post("/", requireAuth, requireRole("admin"), async (req, res) => {
    try {
        const { email, displayName, password, role, payRate } = req.body;
        ...
        const result = await pool.query(`INSERT INTO users (email, password_hash, display_name, role, pay_rate)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING id, email, display_name, role, created_at, pay_rate`, [email.trim().toLowerCase(), passwordHash, displayName.trim(), role, payRate ?? 30]);
...
router.patch("/:employeeId", requireAuth, requireRole("admin"), async (req, res) => {
    try {
      ...
      RETURNING id, email, display_name, role, created_at, pay_rate`, [employeeId, email ? email.trim().toLowerCase() : null,
...
router.delete("/:employeeId", requireAuth, requireRole("admin"), async (req, res) => {
    try {
        const { employeeId } = req.params;
        const result = await pool.query(`DELETE FROM users WHERE id = $1 AND role IN ('admin', 'employee') RETURNING id`, [employeeId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: "Employee not found" });
        }
