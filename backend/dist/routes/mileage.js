...
router.post("/", requireAuth, requireRole("admin"), async (req, res) => {
    try {
        const { employeeId, tripDate, startLocation, endLocation, milesDriven, reimbursementRate, purpose, odometerPhotoDataUrl } = req.body;
...
router.delete("/:id", requireAuth, requireRole("admin"), async (req, res) => {
    try {
        await pool.query(`DELETE FROM mileage_logs WHERE id = $1`, [req.params.id]);
