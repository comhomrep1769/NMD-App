...
router.post("/public", async (req, res) => {
    ...
    try {
        const { firstName, lastName, phone, email, address, serviceType, preferredDate, preferredTime, notes, photoDataUrl, photoNote, waiverAccepted, waiverSignature, client_phone, sms_consent, } = req.body;
      ...
      RETURNING *`, [
        ...
        if (email && client_phone) {
           ...
           WHERE LOWER(email) = LOWER($3)`, [client_phone.trim(), sms_consent ?? false, email.trim()]);
...
router.patch("/:requestId/status", requireAuth, requireRole("admin"), async (req, res) => {
    try {
        ...
        const result = await pool.query(`UPDATE service_requests SET status = $2 WHERE id = $1
      RETURNING id, first_name, last_name, phone, email, address, service_type,
        preferred_date, preferred_time, notes, photo_data_url, photo_note,
        waiver_accepted, waiver_signature, waiver_signed_at, status, created_at`, [requestId, status]);
        if (result.rows.length === 0)
            return res.status(404).json({ error: "Service request not found" });
        const row = result.rows[0];
        await logActivity({
...
router.post("/:requestId/convert-to-client", requireAuth, requireRole("admin"), async (req, res) => {
    try {
        ...
        try {
            ...
            const clientResult = await client.query(`INSERT INTO clients (first_name, last_name, phone, email, address, sms_consent)
        VALUES ($1, $2, $3, $4, $5, $6)
